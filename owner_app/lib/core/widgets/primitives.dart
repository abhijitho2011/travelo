...
class SoftCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget content = Container(
      // The 4px rule replaces a 1px border, so the body shifts by the
      // difference and the text stays on the same optical line as an
      // unaccented card's.
      padding: accent == null
          ? padding
          : padding.copyWith(left: padding.left + 3),
      decoration: BoxDecoration(
        color: c.card,
        borderRadius: R.rLg,
        border: Border.all(color: c.border),
    ...
    if (accent != null) {
      // Flutter refuses to paint a rounded box whose border sides differ in
      // colour, so the accent is a clipped bar drawn over the card rather than
      // a thicker left BorderSide.
      content = Stack(
        children: [
          content,
          Positioned.fill(
...
///
/// Four columns is the ceiling, and the tiles are a little taller than HF's:
/// every owner KPI carries a hint line under the numeral, and at six columns on
/// a landscape tablet the tile grew too short to fit it.
class KpiGrid extends StatelessWidget {
  ...
  /// Width : height of one tile. Shared with [KpiSkeleton] so the placeholder
  /// and the real thing occupy the same space.
  static const double tileAspectRatio = 1.5;
