...
/// HF `<EmptyState>` — a dashed frame with a title and an optional hint.
...
/// Failure state. Distinguishes "we are offline", "this endpoint is not live
/// yet" and a genuine error, because the honest message differs in each case.
class ErrorState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (IconData icon, String title, String body) = switch (api) {
      ...
      final e? when e.isMissingEndpoint => (
        Icons.cloud_off_outlined,
        'Not available yet',
        'This part of Tavelo is not live on the server yet. It will fill in as soon as it is.',
      ),
      final e? when e.code == ApiErrorCodes.forbidden => (
        Icons.lock_outline,
        'Not permitted',
        "Your role doesn't have access to this data.",
      ),
...
/// Renders [data], [ListSkeleton] while loading and [ErrorState] on failure —
/// the one place the three states are wired together.
class AsyncSection<T> extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (value.isLoading) return loading ?? const ListSkeleton();
    final error = value.error;
    if (error != null) return ErrorState(error: error, onRetry: onRetry);
    final v = value.value;
    if (v == null) return const EmptyState(title: 'Nothing to show yet');
    return data(v);
  }
}
...
class AsyncSnapshotLike<T> {
  ...
  const AsyncSnapshotLike.failure(this.error) : value = null, isLoading = false;
